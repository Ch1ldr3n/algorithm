...
void insert_sort(int *first, int *last, Comp comp) {
  for (int *cur = first + 1, *tmp = cur; cur != last; tmp = ++cur) {
    int val = *tmp;
    if (comp(val, *first))
      while (tmp != first) *tmp = *(tmp - 1), --tmp;
    else
      while (comp(val, *(tmp - 1))) *tmp = *(tmp - 1), --tmp;
    *tmp = val;
  }
}
...
void merge_sort(int *first, int *last, Comp comp) {
  if (last - first <= 8) return insert_sort(first, last, comp);
...
// [first, last - 1] is a min heap, except for first and its 2 children may
// violate heap rule
...
void down(It first, It last) {
  ...
  auto val = first[0];
  while (true) {
    if (son + 1 < n && first[son + 1] < first[son]) ++son;
...
void init() {
  for (int i = 1; i <= n; ++i) h[i] = {s2[i], s3[1], 1};
  get(1);
}
}  // namespace rem
...
void get_kth(int n, int k, int *x, int *y, int *z) {
  ::n = n;
  for (int i = 1; i <= n; ++i) s1[i] = s2[i] = s3[i] = i;
  merge_sort(s1 + 1, s1 + n + 1,
             [](int l, int r) { return compare(l, 1, 1, r, 1, 1); });
  merge_sort(s2 + 1, s2 + n + 1,
             [](int l, int r) { return compare(1, l, 1, 1, r, 1); });
  merge_sort(s3 + 1, s3 + n + 1,
             [](int l, int r) { return compare(1, 1, l, 1, 1, r); });
  for (int i = 1; i <= n; ++i) h[i] = {s1[i], s2[1], s3[1], 1};
  rem::init();
  while (--k) {
    Data3 cur = h[1];
