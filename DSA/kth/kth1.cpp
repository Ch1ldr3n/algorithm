...
struct Data {
  ...
  bool operator<(Data obj) {
    return compare(X[x], Y[y], Z[z], X[obj.x], Y[obj.y],
                   Z[obj.z]);  //为啥要加[]
  }
};
template <typename T>
...
inline bool lessThan(int a, int b, int flag) {  //比较x/y/z数组中秩为a,b元素大小
  switch (flag) {
    case 1:
      return compare(a, 1, 1, b, 1, 1);
    case 2:
      return compare(1, a, 1, 1, b, 1);
    default:
      return compare(1, 1, a, 1, 1, b);
  }
}
...
void quickSort(int a[], int lo, int hi, int flag) {
  ...
  for (int k = lo + 1; k <= hi; k++)
    if (lessThan(a[k], pivot, flag)) swap(a[++mi], a[k]);
  swap(a[lo], a[mi]);
...
void get_kth(int n, int k, int *x, int *y, int *z) {
  ...
  while (--k) {
    Data data = h.getMin();
    h.delMin();
    if (data.z != n) h.insert(Data(data.x, data.y, data.z + 1));
    if (data.z == 1) {  //避免重复加入
      if (data.y == 1 && data.x != n)
        h.insert(Data(data.x + 1, data.y, data.z));
      if (data.y != n) h.insert(Data(data.x, data.y + 1, data.z));
    }
  }
  Data res = h.getMin();
