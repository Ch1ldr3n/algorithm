...
void trieDelete(ull num) {
  ...
  if (leaf[j].empty()) {  //从叶节点回溯，维护节点的存在性
    for (int i = 63; i >= 1; i--) {
      trieNode &k = trie[path[i]], &p = trie[path[i - 1]];
      int bit = getBit(num, 63 - i), pbit = getBit(num, 64 - i);
      if (i == 63) {
        if (k.son[!bit] <= 0 || leaf[k.son[!bit]].empty())
...
int main() {
  ...
  for (int i = 0; i < n; ++i) {
    while (r < R && r < n - 1) trieAdd(num[++r], r);
    while (l < L && l < n) trieDelete(num[l++]);
    printf("%d\n", trieFind(num[i], i));
